...
| `avatar.ts` | A pilot's face, five by five pixels mirrored down the middle, drawn from a seed: the hash of their provider id, which the server derives and the provider id itself never leaves it. No photo, no request, no stored image — an entry with no account falls back to the display name. A duel is the one exception: the rival's provider photo is shown over the pixels on the starting grid and beside the verdict, carried from the sign-in fragment through the room and stored by nobody |
| `friends.ts` | The friends block at the top of the duel sheet: challenges received first — there is nothing to do but join them — then the friends to challenge, then the two codes it takes to get linked — mine as a QR as well, since six characters dictate but do not type. The link and its QR stay below, for whoever is not a friend yet, and a duel offers to keep the pilot you just raced |
| `photos.ts` | **The one place that decides which face to show** — `faceHtml` for a row built as HTML, `paintFace` for an element — and the provider photos this device has seen, in `localStorage`, keyed by the face seed. The server keeps none — a duel is where they travel — so the only place that can remember one for the friends list is the device that saw it. The host is re-checked on read, not only on write: the store belongs to the player and what comes out of it ends up in an `<img>`. Keying on the account is what makes a change of account change the face: a single key with no identity kept the previous one, and the menu showed it while the settings showed the new pixels |
...
| `room.ts` | The Durable Object of a duel: owns the seed neither member sees, serves the track in chunks to both, replays each member's trace incrementally from the windows they send at 10 Hz, ejects one whose claimed distance diverges from the replay, and relays six numbers to the other — nothing else ever crosses; sends the start with its countdown when the second socket opens, and the result when both have reached the line or their wreck, ranked in simulated steps, never wall-clock; a duel writes nothing to the board, only a win, a loss and a play to the `duels` counter keyed by the players' ULIDs |
| `friends.ts` | Friends and challenges: a six-character code per account — dictatable, no O/0 or I/1, and the same `shortCode` now names a duel room so pairing works by QR **or** by voice — a friendship that is asked for and accepted, and a challenge that is a room already open, held for ten minutes and found when the friend next opens the game. No presence and no push: `NETWORK.md` says what a heartbeat would cost on the free plan. What leaves here about a pilot is a name, a face and a code, never an account id or an ULID |
