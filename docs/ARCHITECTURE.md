...
| `friends.ts` | The friends block at the top of the duel sheet: challenges received first — there is nothing to do but join them — then the friends to challenge, then the two codes it takes to get linked. The link and its QR stay below, for whoever is not a friend yet |
| `photos.ts` | The provider photos this device has seen, in `localStorage`, keyed by the face seed. The server keeps none — a duel is where they travel — so the only place that can remember one for the friends list is the device that saw it. The host is re-checked on read, not only on write: the store belongs to the player and what comes out of it ends up in an `<img>` |
