...
`tests/layout-doc.test.ts` generates the line counts of `ARCHITECTURE.md` and
the table of item 20 between markers, and fails if a source module is not named
in the map. What it cannot check is a role that is stale rather than absent;
...
| File | Lines |
|---|---|
| `src/client/audio.ts` | 625 |
| `src/client/main.ts` | 517 |
| `src/sim/track.ts` | 398 |
| `src/client/ship.ts` | 377 |
| `src/client/sliders.ts` | 341 |
| `src/sim/step.ts` | 315 |
