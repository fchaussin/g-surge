...
|---|---|
| Keys in `DEFAULTS` | 72 |
| Exposed as sliders | 43 |
| Reachable only through `__gsNext.tuning()` | 29 |
<!-- /generated:coverage -->
...
registration whenever the page becomes visible and reloads on a controller
change — at once in the menu, otherwise at the next return to it, never on the
first install; the worker's navigation fetch asks for `cache: 'no-cache'`, so
...
| File | Lines |
|---|---|
| `src/client/audio.ts` | 697 |
| `src/client/main.ts` | 542 |
| `src/client/ship.ts` | 418 |
| `src/sim/track.ts` | 398 |
| `src/client/sliders.ts` | 376 |
| `src/sim/step.ts` | 328 |
| `src/client/track-mesh.ts` | 313 |
<!-- /generated:oversize -->
