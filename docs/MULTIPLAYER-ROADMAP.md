...
1. **Two ships, one track, live.** *Done, 1.25.0. Server: `room.ts`, proven
   ...
   simulation to the bit, a member who lies on its distance is closed with
   4001. Client: `duel.ts` and the ghost's puppet mode, proven in Playwright
   with the room played by `routeWebSocket` — chunks leave every 72 steps in
   a form the server decodes, the other ship draws thirty metres ahead from
   relays, dead-reckoned between them and eased on the frame clock.* A room
   object owns the track generator
